...
export default function RetranscribeSelection({children}:{children?:(action:{open:()=>void;disabled:boolean})=>ReactNode}){
 const f=useForkI18n();
 ...
 const selection=selected.length?{start:Math.min(...selected.map(w=>w.start)),end:Math.max(...selected.map(w=>w.end))}:clip;
 ...
 const run=async()=>{
  ...
  try{await flushProjectAutosave();await window.rescriptDesktop!.jobs.transcribeRange(s.projectId!,range.start,range.end,model,language);setRange(null);}
 ...
 {range&&createPortal(<div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40" onClick={()=>{if(!busy)setRange(null);}}>
  <div role="dialog" aria-modal="true" aria-label={f('Retranscribe selection')} onClick={e=>e.stopPropagation()} className="w-96 rounded-xl bg-white p-5 text-zinc-900 shadow-xl dark:bg-zinc-900 dark:text-zinc-100">
   <h2 className="mb-2 font-semibold">{f('Retranscribe selection')}</h2>
   <p className="mb-4 text-sm">{range.start.toFixed(2)}–{range.end.toFixed(2)} s</p>
   ...
   <p className="my-3 text-xs text-zinc-500">{f('Only this source range will be replaced. Timeline cuts stay unchanged.')}</p>
