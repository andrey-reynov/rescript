...
export default function RetranscribeSelection({children,scope='selection'}:{scope?:'selection'|'all';children?:(action:{open:()=>void;disabled:boolean})=>ReactNode}){
 ...
 const run=async()=>{
  ...
  try{assertModelLanguage(model,language);await flushProjectAutosave();if(scope==='all')await window.rescriptDesktop!.jobs.transcribeAll(s.projectId!,model,language);else await window.rescriptDesktop!.jobs.transcribeRange(s.projectId!,range.start,range.end,model,language);setRange(null);}
