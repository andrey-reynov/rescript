...
export default function ModelPicker({value,onChange,disabled}:{value:ModelId;language:TranscriptLanguage;onChange:(id:ModelId)=>void;disabled:boolean}){
 ...
 useEffect(()=>{let cancelled=false;const refresh=()=>{void modelAvailability().then(result=>{if(!cancelled)setAvailability(result);});};refresh();const timer=setInterval(refresh,5000);window.addEventListener('focus',refresh);return()=>{cancelled=true;clearInterval(timer);window.removeEventListener('focus',refresh);};},[]);
 ...
 const options=groups.flatMap(group=>MODEL_ORDER.filter(id=>availability[id]===group.status).map(id=>{
  const m=MODELS[id];return {value:id,label:m.label,group:group.label,icon:<AudioLines size={14} className="shrink-0 text-zinc-400"/>,description:f(m.englishOnly?'English only':m.backend==='parakeet'?'25 European languages, including Russian · automatic detection':'Multilingual, including Russian and English'),meta:<span className="flex shrink-0 items-center gap-2 text-[10px] text-zinc-400">{m.experimental&&<span>{f('Experimental')}</span>}{group.status==='missing'&&<span>{m.size}</span>}</span>};
 }));
