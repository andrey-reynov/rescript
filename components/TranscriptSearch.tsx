...
export default function TranscriptSearch(){
  ...
  if(!open)return <button title={f("Find in transcript (Ctrl+F)")} aria-label={f("Find in transcript")} className="rounded p-1 text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800" onClick={()=>{setOpen(true);requestAnimationFrame(()=>input.current?.focus());}}><Search size={15}/></button>;
  return <div className="flex min-w-0 items-center gap-1 text-xs">
    <input ref={input} aria-label={f("Search transcript")} placeholder={f("Find words or a phrase…")} value={query} onChange={e=>{setQuery(e.target.value);setPosition(-1);}} onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();select(e.shiftKey?current-1:position+1);}if(e.key==='Escape'){setOpen(false);}}} className="min-w-0 w-40 rounded border border-zinc-300 bg-transparent px-2 py-1 dark:border-zinc-700"/>
    <span role="status" className="whitespace-nowrap text-zinc-500">{matches.length?`${current+1}/${matches.length}`:f('No matches')}</span>
    <button aria-label={f("Previous match")} disabled={!matches.length} onClick={()=>select(current-1)}><ChevronUp size={14}/></button>
    <button aria-label={f("Go to match")} disabled={!matches.length} onClick={()=>select(current)}><Search size={14}/></button>
    <button aria-label={f("Next match")} disabled={!matches.length} onClick={()=>select(position+1)}><ChevronDown size={14}/></button>
    <button aria-label={f("Close search")} onClick={()=>setOpen(false)}><X size={14}/></button>
  </div>;
}
